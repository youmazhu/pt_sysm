...
class SegVideoDetector(QThread):
    ...
    def run(self):
        try:
            ...
            self.allowed_ids = _normalize_classes(model, classes=None)  # 运行时也可更新
            ...
            output_path = os.path.join(self.save_dir, 'seg_' + os.path.basename(self.video_path))
            ...
            while cap.isOpened() and not self._stop_event.is_set():
                ...
                annotated = results[0].plot()  # 自动叠加掩膜
                out.write(annotated)
            ...
            cap.release()
            out.release()
            self.detection_finished.emit(f'分割完成，结果已保存至 {output_path}')
        except Exception as e:
            self.detection_finished.emit(f'分割出错: {str(e)}')
        finally:
            self.running = False
...
def detect_video_seg(model_path, video_path, save_dir, conf=0.5, iou=0.45, classes=None, target_fps=None, imgsz=640):
    det = SegVideoDetector(model_path, video_path, save_dir, conf, iou, classes, target_fps, imgsz)
    det.start()
    return det
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, default='runs/segment', help='输出目录')
    parser.add_argument('--model', type=str, default='yolov8n-seg.pt', help='分割模型路径')
    parser.add_argument('--conf', type=float, default=0.5, help='置信度阈值')
    parser.add_argument('--iou', type=float, default=0.45, help='IOU阈值')
    parser.add_argument('--classes', type=str, default='', help='类别筛选，如 "ship,boat" 或 "0,2"')
    ...
    det = detect_video_seg(args.model, args.input, args.output_dir, args.conf, args.iou, classes, args.fps, args.imgsz)
    det.wait()