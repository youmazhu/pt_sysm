...
if __name__ == '__main__':
    ...
    parser.add_argument('--show', action='store_true', help='实时显示检测结果（需要 GUI 支持）')
    ...
    # 逐帧处理
    display_enabled = bool(args.show)
    display_used = False
    display_failed_logged = False
    ...
    while cap.isOpened():
        ...
        if display_enabled:
            try:
                ...
                if not display_failed_logged:
                    print('HighGUI 不可用，自动禁用实时预览。')
                    display_failed_logged = True
    ...
    out.release()
    try:
        if display_used and hasattr(cv2, 'destroyAllWindows'):
            cv2.destroyAllWindows()
    except Exception:
        pass
